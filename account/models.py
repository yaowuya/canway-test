...
class BkUser(AbstractBaseUser, PermissionsMixin):
    ...
    def to_dict(self):
        temp_dict =  dict([(attr, getattr(self, attr)) for attr in [f.name for f in self._meta.fields]])
        temp_dict['date_joined'] = temp_dict['date_joined'].strftime('%Y-%m-%d %H:%M:%S')
        temp_dict['last_login'] = temp_dict['last_login'].strftime('%Y-%m-%d %H:%M:%S')
        return temp_dict